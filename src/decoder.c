...
uint8_t zzindex[64] = {  0,  1,  5,  6, 14, 15, 27, 28,
...
void anti_q (BLOCK obj, DQT* quant) {
    // Inverse quantize
    //printf("first element of quantizer: %hu\n", quant->quantizer[0]);
    //printf("length of quantizer: %d\n", sizeof(quant->quantizer)/sizeof(quant->quantizer[0]));
    //printf("size of a block: %d\n", sizeof(BLOCK));
    //printf("length of a block: %d\n", sizeof(BLOCK)/sizeof(obj[1]));
    for(int i=0;i<64;++i){
...
BLOCK* read_BLOCK (JPEG_INFO* info, u8 chan_id) {
    ...
    while(counter<64)
    {
        ...
        u8 ac_zeros = symbol_len >> 4;
        u8 ac_codelen = symbol_len & 0x0F;
        //printf("ac code length: %d\n", ac_codelen);        
        if(ac_zeros==15 && ac_codelen==0)
        {
            for(int i=0;i<16;++i){
                (*obj)[counter] = 0;
                ++counter;
        ...
        else if(ac_codelen==0){
            //printf("Is zero\n");
            break;
        }
        else{
            for(int i=0;i<ac_zeros;++i){
                (*obj)[counter] = 0;
                ++counter;
            }
            (*obj)[counter]=get_symbol(ac_codelen, info->fp);
            ++counter;
        }
    }
    while(counter<64){
